...
이 문제를 해결하려면 `KV.Registry.create/2`에서 `cast/2` 대신에 `call/2`를 사용하여 동기적으로 동작하게끔 만들어야 합니다. 이는 변경사항이 테이블에 반영된 뒤에 클라이언트에서 원하는 후속작업을 할 수 있게끔 보장할 것입니다. 그러면 다음과 같이 변경해봅시다:
