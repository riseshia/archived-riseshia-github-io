...
이름 저장소(registry)는 사전이 언제나 최신임을 보장할 필요가 있습니다. 예를 들어, 저장 공간 프로세스가 하나라도 버그 때문에 동작을 멈추게 된다면, 이름 저장소는 잘못된 정보를 제공하지 않기 위해서 사전을 정리할 필요가 있습니다. 이를 Elixir에서는 이름 저장소가 각 저장소를 감시해야한다고 표현하죠.
...
그럼 이제 다시 서버의 콜백을 고쳐서 이 문제를 해결해 봅시다. 우선, GenServer의 상태를 다음의 두 사전으로 나눌 것입니다: 하나는 `name -> pid`이며, 나머지 하나는 `ref -> name`입니다. 그리고 `handle_cast/2`를 통해 생성된 키-값 저장소를 감시하게끔 만들고, `handle_info/2` 콜백을 구현해서 관련된 메시지를 처리할 수 있도록 합시다. 완전한 서버 콜백 구현은 다음과 같습니다:
